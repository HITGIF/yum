...
module M = struct
  ...
  let process (env : Eio_unix.Stdenv.base) { token } { name; meth; body; url } : reply =
...
let get_guild_member ~user_id ~guild_id t =
  request __FUNCTION__ ~meth:`GET ("/guilds/" ^ guild_id ^ "/members/" ^ user_id) t
...
let get_guild_roles ~guild_id t =
  ...
  |> Result.map (fun x -> x |> Yojson.Safe.Util.to_list |> List.map Entity.role_of_yojson)
;;
