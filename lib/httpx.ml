...
module Ws = struct
  ...
  let drain_handshake req ic oc nonce =
    ...
    then failwith ("error status: " ^ Cohttp.Code.(string_of_status status));
    if Cohttp.Response.version resp <> `HTTP_1_1 then failwith "invalid HTTP version";
    ...
    if not (Websocket.upgrade_present headers) then failwith "upgrade header not present";
    (match Cohttp.Header.get headers "sec-websocket-accept" with
     | Some accept
       when accept = Websocket.b64_encoded_sha1sum (nonce ^ Websocket.websocket_uuid) ->
       ()
