  ...
  object (self)
    ...
    method resume_ws env ~sw state =
      ...
      let url =
        ...
        let u = Uri.with_query u [ "v", [ "10" ]; "encoding", [ "json" ] ] in
        Uri.to_string u
      in
      ...
      Ws.Process.start ~sw conn (self :> _ Actaa.Process.t1);
      (* Send Resume event *)
      Event.(Resume { token = state.token; session_id; seq = state.seq } |> to_yojson)
      |> send_json conn;
      { state with ws_conn = conn }

    method private init env ~sw { consumer; token; intents; state = st } =
      let ws_conn = self#connect_ws env ~sw in
      make_state ~consumer ~ws_conn ~token ~intents ~st ~seq:0 ~identify_sent:false ()
    ...
    method handle_event env ~sw state =
      ...
      | Hello { heartbeat_interval } ->
        ...
        then
          ...
          |> to_yojson
          |> send_json state.ws_conn;
        { state with heartbeat_interval = Some interval; identify_sent = true }
      | HeartbeatAck ->
      ...
      | Dispatch
        ...
        then (
          ...
          | Some channel_id ->
            ...
            if State.set_voice_if_not_exists state.st ~guild_id ~channel_id ~gateway:vgw
      ...
      | Dispatch (GUILD_CREATE { voice_states; id = guild_id; _ }) ->
        voice_states
        |> Option.iter (State.initialize_guild_voice_states state.st ~guild_id);
        state
      ...
      | `WSText (content, _) ->
        (try
         ...
         | e ->
           Logs.err (fun m ->
             m
               "Handling event failed: %s: %s: %s"
               (Printexc.to_string e)
               content
               (Printexc.get_backtrace ()));
           `NoReply state)
      | `WSClose (_, conn) when Ws.id state.ws_conn = Ws.id conn ->
        Logs.info (fun m -> m "Gateway WS connection closed. Trying to resume.");
