...
module Voice_states = struct
  ...
  type msg = basic_msg
  type state = { store : Event.dispatch_voice_state_update StringMap.t StringMap.t }
    ...
    object
      ...
      method! private handle_call _env ~sw:_ _from state =
        ...
        | `Get (guild_id, user_id) ->
          let vstate =
            let ( >>= ) = Option.bind in
            StringMap.find_opt guild_id state.store >>= StringMap.find_opt user_id
          in
        ...
        | `SetAll (guild_id, src) ->
          let store =
            ...
            |> StringMap.update guild_id
               ...
               | None ->
                 src
                 |> List.map (fun (vstate : Event.dispatch_voice_state_update) ->
...
module Voice = struct
  ...
  type call_msg =
    [ `Register of key * process
    | `Lookup of string (* guild_id *)
    ]
  ...
  type state =
    { r : R.t
    ; m : string StringMap.t (* guild_id -> channel_id *)
    }
    ...
    object
      ...
      method! private handle_call _env ~sw:_ _from state =
        ...
        | `Register (key, process) ->
          ...
          let state =
            if is_correctly_registered
            then { state with m = state.m |> StringMap.add (fst key) (snd key) }
            else state
          in
...
let set_voice_if_not_exists { voice; _ } ~guild_id ~channel_id ~gateway =
  match Actaa.Gen_server.call voice (`Register ((guild_id, channel_id), gateway)) with
  | `Register v -> v
  | _ -> assert false
;;
