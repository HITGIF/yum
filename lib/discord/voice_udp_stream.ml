...
let send_speaking state v = Actaa.Gen_server.cast state.vgw (`Speaking (state.ssrc, v))
...
let send_frame state frame =
  ...
  { state with seq_num = state.seq_num + 1; timestamp = state.timestamp + frame_size }
;;
  ...
  | Some src ->
    ...
    let frames =
      List.init
        ...
        (fun i -> Cstruct.sub buf (i * frame_len) frame_len |> Cstruct.to_string)
    in
    (* Send the frames *)
    let state =
      frames
      |> List.fold_left (fun state frame -> send_frame state (`PCM_S16LE frame)) state
    in
    (* Sleep while sending *)
    let end_time = Eio.Time.now clock in
    let diff = end_time -. start_time in
    let seconds = (second_per_frame *. float_of_int (List.length frames)) -. diff in
    ...
    then state
      ...
      let state =
        five_silent_frames
        |> List.fold_left (fun state opus -> send_frame state (`Opus opus)) state
      in
...
class t =
  object (self)
    ...
    method private init env ~sw { ip; port; ssrc; vgw } =
      let socket =
        Eio.Net.datagram_socket ~sw (Eio.Stdenv.net env) (`Udp (Eio.Net.Ipaddr.V4.any, 0))
      in
      let dst =
        `Udp
          ( Ipaddr.of_string ip
      ...
      let opus_encoder = Opus.Encoder.create ~sample_rate ~channels ~application in
    ...
    method! private handle_info env ~sw state =
      ...
      | `Timeout _ ->
        let state =
          start_sending_frames_from_source env ~sw state (self :> _ Actaa.Timer.receiver)
        in
        `NoReply state
    ...
    method! private handle_cast env ~sw state =
      ...
      | `FrameSource src ->
        let state = { state with queued_sources = Fqueue.add state.queued_sources src } in
        (match state.speaking_source with
         | Some _ -> `NoReply state
         | None ->
           let state =
             start_sending_frames_from_source
               env
               ~sw
               state
...
let discover_ip (t : t) =
  match Actaa.Gen_server.call t `DiscoverIP with
  | `DiscoverIP r -> r
;;
