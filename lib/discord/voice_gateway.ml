...
let send_select_protocol conn ip port modes =
  if modes |> List.find_opt (fun m -> m = encryption_mode) = None
  then failwith (encryption_mode ^ " is not supported");
  Voice_event.(
    SelectProtocol
      { protocol = "udp"; data = { address = ip; port; mode = encryption_mode } }
    |> to_yojson)
  |> send_json conn
;;
...
class t =
  object (self)
    ...
    method private connect_ws env ~sw state =
      ...
      let conn = Ws.connect ~sw env ("https://" ^ endpoint ^ "/?v=4&encoding=json") in
    ...
    method private start_running env ~sw state =
      ...
      Voice_event.(
        Identify { server_id = state.guild_id; user_id; session_id; token } |> to_yojson)
      |> send_json conn;
    ...
    method private handle_voice_event env ~sw state =
      ...
      | Voice_event.Hello { heartbeat_interval } ->
        ...
        if Option.is_none state.heartbeat_interval
        then
          Actaa.Timer.spawn
            env
            ~sw
      ...
      | Ready { ip; port; ssrc; modes; _ } ->
        ...
        Voice_udp_stream.connect env sw state.udp_stream { vgw; ip; port; ssrc; modes };
        let my_addr, my_port = Voice_udp_stream.discover_ip state.udp_stream in
    ...
    method! private handle_info env ~sw state =
      ...
      | `WSText (content, _) ->
        (try
           ...
           |> Voice_event.of_yojson
           |> self#handle_voice_event env ~sw state
           |> fun state -> `NoReply state
         with
         | e ->
           Logs.err (fun m ->
             m "Handling event failed (voice): %s: %s" (Printexc.to_string e) content);
           `NoReply state)
    ...
    method! private handle_cast env ~sw state =
      ...
      | `Speaking (ssrc, speaking) ->
        ...
        in
        Voice_event.(
          Speaking { speaking = (if speaking then 1 else 0); delay = Some 0; ssrc }
          |> to_yojson)
        |> send_json (Option.get state.ws_conn);
        Actaa.Gen_server.cast
          state.consumer
          (`Event (Event.VoiceSpeaking { guild_id = state.guild_id; speaking }));
        `NoReply state
      | `Stop ->
