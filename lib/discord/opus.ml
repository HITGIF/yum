module Low_level = struct
  ...
  let encoder_create =
    foreign "opus_encoder_create" (int32 @-> int @-> int @-> ptr int @-> returning encoder)
  ;;
  ...
  let encoder_destroy = foreign "opus_encoder_destroy" (encoder @-> returning void)
end
...
module Encoder = struct
  ...
  let create ~sample_rate ~channels ~application =
    ...
    then failwith "Only 48kHz stereo is supported";
    let open Ctypes in
    let application =
    ...
    let encoder = Low_level.encoder_create sample_rate channels application error in
    if !@error <> 0
    then failwith (Printf.sprintf "Failed to create Opus encoder: %d" !@error);
  ...
  let encode ?(frame_size = 960) encoder (`S16LE pcm_s16le) =
    ...
    if String.length pcm_s16le <> pcm_size_in_bytes
    then failwith "Invalid frame and pcm size";
    let open Ctypes in
    let pcm =
      pcm_s16le |> CArray.of_string |> CArray.start |> to_voidp |> from_voidp short
    in
