...
let to_yojson = function
  | Heartbeat s ->
    `Assoc [ "op", `Int 1; "d", s |> Option.fold ~none:`Null ~some:(fun s -> `Int s) ]
  | Identify { token; properties; intents } ->
    `Assoc
      [ "op", `Int 2
      ; ( "d"
        , `Assoc
            [ "token", `String token; "properties", properties; "intents", `Int intents ]
        )
      ]
    ...
    `Assoc
      ...
      ; ( "d"
        , `Assoc
            ...
            ; ( "channel_id"
              , channel_id |> Option.fold ~none:`Null ~some:(fun s -> `String s) )
  ...
  | Resume { token; session_id; seq } ->
    `Assoc
      ...
      ; ( "d"
        , `Assoc
            [ "token", `String token; "session_id", `String session_id; "seq", `Int seq ]
        )
      ]
